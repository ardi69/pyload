...
def generate_and_add(urls, paused):
    packs = API.generatePackages(urls)
    for name, urls in packs.iteritems():
        API.addPackage(name, urls, paused=paused)
...
def add(request):
    ...
    if package:
        API.addPackage(package, urls, paused=True)
    else:
...
def addcrypted():
    ...
    dlc_path = path.join(pycore.config.get('general', 'download_folder'), safe_filename(package) + ".dlc")
    dlc_file = open(dlc_path, "wb")
    dlc_file.write(dlc)
    dlc_file.close()

    try:
        API.addPackage(package, [dlc_path], paused=True)
    except:
        return HTTPError()
    else:
        return "success\r\n"
...
def addcrypted2():
    ...
    try:
        if package:
            API.addPackage(package, result, paused=True)
        else:
...
def flashgot():
    ...
    if package:
        API.addPackage(package, urls, paused=autostart)
    else:
