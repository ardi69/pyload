...
def deprecated(by=None):
    def wrapper(old_fn):
        def new(*args, **kargs):
            if by:
                new_fn = by
                pycore.log.debug(_('"%s" has been Deprecated, use "%s" instead.')
                    % (old_fn.__name__, new_fn.__name__))
                return new_fn(*args, **kargs)
            else:
                pycore.log.error(_('"%s" has been Removed.') % old_fn.__name__)
                traceback.print_exc()
        return new
    return wrapper
    ...
    def wrapper(fn):
        def new(*args, **kwargs):
          try:
              ...
              pycore.log.error(_('Error executing "%s": %s') % (fn.__name__, str(e)))
              if fallback:
                  return fallback
              else:
                  traceback.print_exc()
